...
def call_mcp_tool_by_name(
    ...
    while attempt < max_retries:
        ...
        try:
            ...
            logger.warning(
                f"Attempt {attempt} failed for '{real_tool_name}': {e} "
                f"({'retryable' if retryable else 'not retryable'})"
            )
            if not retryable:
                break  # permanent-looking failure: don't waste retries on it
            if attempt < max_retries:
                time.sleep(DEFAULT_RETRY_BACKOFF_BASE_SECONDS * attempt)

    error_msg = f"{type(last_exc).__name__}: {last_exc}"
    logger.error(f"Error calling tool '{real_tool_name}' after {attempt} attempt(s): {error_msg}")
